    ...
    : IConsumer<PublicationStatusChanged>
{
    public async Task Consume(ConsumeContext<PublicationStatusChanged> context)
    {
        ...
        await hubContext.Clients.User(msg.UserId).OnPublicationStatusChanged(
            msg.PublicationRecordId,
            msg.Status.ToString(),
            msg.PublicUrl,
            msg.PublicationErrorMessage);

        if (msg.Status.ToString().Equals("Published", StringComparison.OrdinalIgnoreCase)
            && Guid.TryParse(msg.UserId, out var userId))
        {
            await SendTelegramIfEnabledAsync(userId, context.CancellationToken);
        }
    }

    private async Task SendTelegramIfEnabledAsync(Guid userId, CancellationToken ct)
    {
        var settings = await db.NotificationSettings
            .FirstOrDefaultAsync(s => s.UserId == userId, ct);

        if (settings?.TelegramChatId is null) return;
        if (!settings.NotifyOnPublication) return;
        ...
        await telegram.SendMessageAsync(settings.TelegramChatId.Value,
            "📢 Публикация в социальную сеть прошла успешно!", ct);
    }
}
